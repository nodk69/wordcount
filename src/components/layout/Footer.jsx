...
import { FOOTER_RESOURCES, FOOTER_LEGAL } from '../../config/navigation';
    ...
    <div>
      ...
      <ul className={`overflow-hidden transition-all duration-300 ease-in-out sm:!max-h-none sm:!opacity-100 space-y-2 mt-3 ${open ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 sm:max-h-none sm:opacity-100'}`}>
        {links.map(({ label, path }) => (
          <li key={label}>
            <Link to={path} className="text-sm text-gray-600 dark:text-gray-400 hover:text-primary transition-colors">{label}</Link>
          </li>
        ))}
      </ul>
    </div>
  ...
  <footer className="bg-gray-50 dark:bg-dark-card border-t border-gray-200 dark:border-dark-border mt-auto">
    <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
        ...
        {/* Resources */}
        <FooterGroup title="Resources" links={FOOTER_RESOURCES} />

        {/* Legal */}
        <FooterGroup title="Legal" links={FOOTER_LEGAL} />
      </div>
